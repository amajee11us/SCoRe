...
class PaCoGCLoss(nn.Module):
    def __init__(self, alpha, 
                       beta=1.0, gamma=1.0, 
                       supt=1.0, temperature=1.0, 
                       base_temperature=None, 
                       is_cf=True,
                       K=8192, num_classes=1000, smooth=0.0):
        super(PaCoGCLoss, self).__init__()
    ...
    def forward(self, features, labels=None, sup_logits=None):
        ...
        # Calculate the Log probabilities based on graph-cut
        if self.is_cf:
            # C_f formulation
            log_prob = torch.div(
