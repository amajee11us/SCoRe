...
class Trainer(object):
    def __init__(self, args, model=None,
                       train_loader=None, val_loader=None,weighted_train_loader=None,
                       per_class_num=[],
                       log=None, loss_version = 'simplified'):
        self.args = args
        ...
        if self.loss_version == 'FL':
            self.comb_loss = FacilityLocation(metric='cosine', lamda=1.0, temperature=0.7) 
        elif self.loss_version == 'GC':
            self.comb_loss = GraphCut(metric='cosine', lamda=1.0, temperature=0.7)
        elif self.loss_version == 'LogDet':
            self.comb_loss = LogDet(metric='cosine', lamda=1.0, temperature=0.7)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (inputs, targets) in enumerate(self.train_loader):
                ...
                output_1, output_cb_1, z1, p1 = self.model(mix_x, train=True) # MixUp based model
                output_2, output_cb_2, z2, p2 = self.model(cut_x, train=True) # CutMix based model
                # contrastive_loss = self.SimSiamLoss(p1, z2, self.loss_version) +\
                #                    self.SimSiamLoss(p2, z1, self.loss_version)
                contrastive_loss = self.SimSiamLoss(p1, z2, target_org, torch.max(mixup_y, dim=-1)[0], self.loss_version) +\
                                   self.SimSiamLoss(p2, z1, target_org, torch.max(mixcut_y, dim=-1)[0], self.loss_version)
    ...
    def SimSiamLoss(self,p, z, target=None, loss_weight=None, version='simplified'):  # negative cosine similarity
